...
public abstract class AbstractPlayerConnector implements Serializable, IPlayerConnector {
  ...
  /**
   * Notify the player an update. Different delivery strategies are applied based on the {@link
   * AbstractMessage} dynamic type. This blocks undefinably until the queue is available (in case
   * socket is used). {@link Game} model instances should leverage this queue to send responses to
   * the client (i.e. game updates).
   *
  ...
  public boolean equals(Object obj) {
    ...
    return casted.getPlayer().equals(player) && casted.getGameId().equals(gameId);
  }
