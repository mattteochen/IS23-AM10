...
public final class Score implements Serializable {
  /** Integer referencing the extra point given to the first player to complete their bookshelf. */
  private Integer extraPoint;

  /**
   * Integer referencing the points the player receives from the groups of same type tiles in their
   * bookshelf.
   */
  private Integer bookshelfPoints;

  /** Integer referencing the points the player receives from completing shared goals. */
  private Integer scoreBlockPoints;

  /**
   * Integer referencing the points the player receives from completing their private (secret) goal.
   */
  private Integer privatePoints;

  /** Map that allows the conversion from number of matches in private cards to points received. */
  public static final Map<Integer, Integer> privateCardPointsMap =
      Map.of(
          0, 0,
          1, 1,
          2, 2,
  ...
  /**
   * Map that allows the conversion from number of groups in player's bookshelf to points received.
   */
  private static final Map<Integer, Integer> bookshelfPointsMap =
      Map.of(
          3, 2,
          4, 3,
          5, 5,
  ...
  /** Integer representing the minimum group size for counting bookshelf points. */
  private static final Integer MIN_GROUP_SIZE = 3;

  /**
   * Integer representing the maximum "useful" group size for counting bookshelf points. Bigger
   * groups count as groups of MAX_GROUP_SIZE
   */
   ...
   * @throws NullPointerException Generic NPE.
   */
  public void setPrivatePoints(Bookshelf bs, PrivateCard pc)
      throws NullPointerException,
  ...
  /**
   * Helper function to help with BFS in bookshelf to find groups of same tile type. NOTE: this
   * function should be called from setBookshelfPoints()
   *
   ...
   * @throws NullIndexValueException If the index provided is null.
   */
  private int countAdjacentTilesRecursive(
      Bookshelf bs, int row, int col, Integer[][] visitedMap, TileType type)
      throws BookshelfGridColIndexOutOfBoundsException,
          BookshelfGridRowIndexOutOfBoundsException,
          NullIndexValueException {
    // Check if the current tile is within the grid bounds and has the same type as the original
    // tile, is not empty not visited
    if (row < 0
        || row >= Bookshelf.BOOKSHELF_ROWS
        || col < 0
        || col >= Bookshelf.BOOKSHELF_COLS
  ...
  /**
   * Getter method that returns the total score computing it from all the available scores.
   *
   * @return total score.
   */
  ...
  /**
   * Method used to retrieve the total score from a possibly obfuscated score. It ignores the
   * invalidated privatescore. Used for sorting purposes in CLI.
   *
   * @return visible score.
   */
  public Integer getVisibleScore() {
    return (privatePoints != -1)
        ? getTotalScore()
        : extraPoint + scoreBlockPoints + bookshelfPoints;
  }

  /**
   * Method used to retrieve a string representing the total score. If score is obfuscated (private
   * card points) it returns a partial score and a graphic indication of possibly missing points.
   *
  ...
  public String getStringTotalScore() {
    return (privatePoints != -1)
        ...
        : String.format("%d (+?)", getVisibleScore());
  }
  ...
  public boolean equals(Object obj) {
    ...
    Score sc = (Score) obj;
    return (extraPoint == sc.getExtraPoint()
  ...
  @Override
  public int hashCode() {
    return extraPoint.hashCode()
        * scoreBlockPoints.hashCode()
        * privatePoints.hashCode()
        * bookshelfPoints.hashCode();
  }
}
