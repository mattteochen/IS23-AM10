 ...
 *
 * <p>NOTE: if not specified, each iteration of the player's bookshelf inside the functions is gonna
 * be first over rows,then columns
 *
...
public final class SharedPatternFactory {
  ...
  /**
   * #1 in rulebook, #4 in images Rule that checks if there are at least six couples of the same
   * tile type in adjacent positions (row or column).
   */
  public static final Predicate<Bookshelf> checkTwoAdjacent =
      bs -> {
        int count = 0;
   ...
   *
   */
  public static final Predicate<Bookshelf> checkCornersMatch =
      bs -> {
        Tile[][] grid = bs.getBookshelfGrid();
        if (grid[0][0].isEmpty()) {
          return false;
  ...
  /**
   * #3 in rulebook, #8 in images Rule that checks if there are at least 4 separate groups of 4
   * elements of the same type in adjacent positions(row or column).
   */
  public static final Predicate<Bookshelf> checkFourAdjacent =
      bs -> {
        int count = 0;
        Tile[][] grid = bs.getBookshelfGrid();
        Set<String> coordsAdjAlreadyCounted = new HashSet<>();
          ...
          for (int j = 0; j < grid[i].length; j++) {
            if (i < grid.length - 3
                ...
                && (!coordsAdjAlreadyCounted.contains(String.valueOf(i) + String.valueOf(j))
                    ...
                    && !coordsAdjAlreadyCounted.contains(
                        String.valueOf(i + 3) + String.valueOf(j)))) {
              coordsAdjAlreadyCounted.add(String.valueOf(i) + String.valueOf(j));
              coordsAdjAlreadyCounted.add(String.valueOf(i + 1) + String.valueOf(j));
            ...
            if (j < grid[i].length - 3
                ...
                && (!coordsAdjAlreadyCounted.contains(String.valueOf(i) + String.valueOf(j))
                    ...
                    && !coordsAdjAlreadyCounted.contains(
                        String.valueOf(i) + String.valueOf(j + 3)))) {
              coordsAdjAlreadyCounted.add(String.valueOf(i) + String.valueOf(j));
              coordsAdjAlreadyCounted.add(String.valueOf(i) + String.valueOf(j + 1));
  ...
  /**
   * Support function that checks if there's a 2x2 square of tiles with the same type.
   *
  ...
  private static final boolean isValidSquare(int startRow, int startCol, Tile[][] grid) {
    ...
    return (grid[startRow][startCol].equals(grid[startRow + 1][startCol])
        ...
        && grid[startRow][startCol].equals(grid[startRow + 1][startCol + 1])
        && currType != TileType.EMPTY);
  }
   ...
   *
   */
  public static final Predicate<Bookshelf> checkSquares =
      bs -> {
        Tile[][] grid = bs.getBookshelfGrid();
  ...
  /**
   * #5 both in rulebook and images Rule that checks if there are at least three columns containing
   * maximum 3 different types of tiles.
   */
  public static final Predicate<Bookshelf> checkMaxThreeTypesInColumn =
      bs -> {
   ...
   *
   */
  public static final Predicate<Bookshelf> checkEightOfSameType =
      bs -> {
        Tile[][] grid = bs.getBookshelfGrid();
        EnumMap<TileType, Integer> checkCount = new EnumMap<>(TileType.class);
        for (int i = 0; i < grid.length; i++) {
  ...
  private static final boolean checkDescDiagonal(int startingRowOffset, Tile[][] grid) {
    ...
    for (int i = 0; i < grid[0].length - 1; i++) {
      if (!grid[grid.length - 1 - i - startingRowOffset][i].equals(
              grid[grid.length - 2 - i - startingRowOffset][i + 1])
  ...
  /**
   * #7 in rulebook, #11 in images Rule that checks if the diagonals are filled with tiles of the
   * same type.
   */
  public static final Predicate<Bookshelf> checkDiagonalsSameType =
      bs -> {
        Tile[][] grid = bs.getBookshelfGrid();
  ...
  /**
   * #8 in rulebook, #7 in images Rule that checks if there are maximum three different types in at
   * least 4 rows.
   */
  public static final Predicate<Bookshelf> checkMaxThreeTypesInRow =
      bs -> {
        int countRows = 0;
        Tile[][] grid = bs.getBookshelfGrid();
        for (int i = 0; i < grid.length; i++) {
  ...
  /**
   * #9 in rulebook, #2 in images Rule that checks if there are at least two columns with all the
   * elements of different type.
   */
  public static final Predicate<Bookshelf> checkTwoColumnAllDiff =
      bs -> {
        int countColumns = 0;
        Tile[][] grid = bs.getBookshelfGrid();
        /*
  ...
  /**
   * #10 in rulebook, #6 in images Rule that checks if there are at least two rows full of different
   * types of tiles.
   */
  public static final Predicate<Bookshelf> checkTwoRowsAllDiff =
      bs -> {
        int countRows = 0;
        Tile[][] grid = bs.getBookshelfGrid();
        for (int i = 0; i < grid.length; i++) {
  ...
  /**
   * Support function that checks, starting from a tile in the grid, if it's part of an X shape of
   * tiles with the same type.
   *
  ...
  /**
   * #11 in rulebook, #10 in images Rule that checks if there are 5 tiles of the same type on a 'X'
   * shape.
   */
  public static final Predicate<Bookshelf> checkTilesXShape =
      bs -> {
        Tile[][] grid = bs.getBookshelfGrid();
  ...
  /**
   * #12 both in rulebook and images Rule that checks if the columns in the bookshelf are ordered
   * (asc o desc) and the rest of the bookshelf is filled with {@link TileType#EMPTY}.
   */
  public static final Predicate<Bookshelf> checkOrderedBookshelfColumns =
      bs -> {
        Tile[][] grid = bs.getBookshelfGrid();

        Predicate<Tile[][]> checkDescOrder =
            g -> {
        ...
        Predicate<Tile[][]> checkDescOrderShifted =
            g -> {
              for (int i = 0; i < g.length; i++) {
                for (int j = 0; j < g[0].length; j++) {
        ...
        Predicate<Tile[][]> checkAscOrderShifted =
            g -> {
              for (int i = 0; i < g.length; i++) {
                for (int j = 0; j < g[0].length; j++) {
        ...
        return (checkAscOrder.test(grid)
            || checkDescOrder.test(grid)
            || checkAscOrderShifted.test(grid)
            || checkDescOrderShifted.test(grid));
      };

  /**
   * The list of {@link SharedPattern} containing all the 12 different patterns. rules with their
   * lambda functions
   */
  private static final List<SharedPattern<Predicate<Bookshelf>>> patternsArray =
      List.of(
   ...
   *
   * @param usedPatterns a List of {@link SharedPattern} storing the already used patterns.
  ...
  public static SharedPattern<Predicate<Bookshelf>> getNotUsedPattern(
    ...
    if (usedPatterns.isEmpty()) {
      ...
      List<SharedPattern<Predicate<Bookshelf>>> unusedPatterns =
          patternsArray.stream()
