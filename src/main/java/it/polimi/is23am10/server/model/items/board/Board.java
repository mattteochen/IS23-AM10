...
public class Board implements Serializable {
  ...
  /**
   * A fixed 2d array referencing a weight for each boardGrid cell. Each value represents the
   * minimum number of players needed to get that square filled.
   *
   * <ul>
  ...
  public Board(Board toCopy) {
    ...
    boardGrid = Arrays.stream(toCopy.boardGrid).map(Tile[]::clone).toArray(Tile[][]::new);
  ...
  /**
   * Extract a tile from the sack. This mutate the sack list.
   *
  ...
  /** Create the board tile sack. The complete sack has 22 tiles for each {@link TileType}. */
  private void createInitialTileSack() {
    tileSack =
        Stream.of(TileType.values())
  ...
  /**
   * Fill the board grid based on the current player number. Note that this method works both when
   * first filling the board AND when re-filling it partially mid-game.
   */
  ...
  /**
   * Retrieve the tile in a specific board position. This method removes the returned tile from the
   * sack.
   *
   * @param row The row index.
   * @param col The column index.
  ...
  /**
   * Method called at the end of the turn that checks if the Board needs to be refilled and proceeds
   * if so.
   */
