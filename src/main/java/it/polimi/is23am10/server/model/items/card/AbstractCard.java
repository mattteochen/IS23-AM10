...
public abstract class AbstractCard<R, T extends AbstractPattern<R>> implements Serializable {
  ...
  /** The pattern instance. Its type must extends {@link AbstractPattern} */
  private transient T pattern;

  /**
   * Pattern setter. Pattern can not be final as it must be randomly chosen and it has to be unique
   * across all instantiated cards. Only the derived class can set the pattern.
   *
   ...
   *     If the pattern is already initiated.
   */
  protected void setPattern(T pattern) throws AlreadyInitiatedPatternException {
    if (this.pattern != null) {
      throw new AlreadyInitiatedPatternException(
          "[Class "
              + this.getClass()
              + ", method setPattern]: The pattern has already been instantiated");
    }
