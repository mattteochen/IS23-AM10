...
public final class CommandLineInterface implements UserInterface, Serializable {
    ...
    if (availableGames.isEmpty()) {
      ...
      for (VirtualView ag : availableGames) {
        String disconnectedPlayers =
            (ag.getDisconnectedPlayersNum() > 0)
                ? String.format(CLIStrings.disconnectedPlayers, ag.getDisconnectedPlayersNum())
                : "";
        ow.info(
            String.format(
                CLIStrings.availableGameString,
                availableGames.indexOf(ag),
                ag.getPlayers().size(),
                ag.getMaxPlayers(),
                disconnectedPlayers,
                ag.getGameId()),
  ...
  public String getFormattedChatMessage(ChatMessage message) {
    ...
    if (message.isBroadcast()) {
      ...
      try {
        if (message
            .getReceiverName()
            .equals(Client.getPlayerConnector().getPlayer().getPlayerName())) {
  ...
  public void displayError(ErrorMessage errorMessage) {
    ...
    final String msgString =
        errorMessage.isBroadcast() ? CLIStrings.broadcastErrorString : CLIStrings.errorMessage;
  ...
  /**
   * CLI method to retrieve user input and add it to the queue for it to be consumed when needed by
   * controller.
   */
