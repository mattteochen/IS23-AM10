...
public class MoveCommandHelperTest {
    ...
    @Test
    public void moveCommandConverter_should_convert_correctly() throws BookshelfGridColIndexOutOfBoundsException, BookshelfGridRowIndexOutOfBoundsException, NullIndexValueException, WrongBookShelfPicksException, NullTileException{
        ...
        List<Coordinates> coordsEmptyBs = List.of(new Coordinates(emptyBs.getBookshelfGrid()[0].length,0));
        List<Coordinates> coords = List.of(
            new Coordinates(emptyBs.getBookshelfGrid()[0].length - 1,1),
            new Coordinates(emptyBs.getBookshelfGrid()[0].length - 2,1));

        assertEquals(coordsEmptyBs,MoveCommandHelper.fromColIdxToCoord(columnIdxEmptyBs, emptyBs, nMovesEmptyBs));
