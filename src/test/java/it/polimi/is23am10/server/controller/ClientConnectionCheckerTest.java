...
class ClientConnectionCheckerTest {
  ...
  @Test
  void CHECK_ALL_PLAYERS_CONNECTION_should_PERFORM_CONNECTION_CHECK()
      throws NullSocketConnectorException, NullBlockingQueueException, GameSnapshotUpdateException {
  ...
  void CHECK_ACTIVE_PLAYERS_ACTIVITY_should_PERFORM_EXPIRATION_CHECK()
    ...
    AbstractPlayerConnector steve =
        new PlayerConnectorSocket(new Socket(), new LinkedBlockingQueue<>());
    AbstractPlayerConnector mike =
        new PlayerConnectorSocket(new Socket(), new LinkedBlockingQueue<>());
    AbstractPlayerConnector alice =
        new PlayerConnectorSocket(new Socket(), new LinkedBlockingQueue<>());
    ...
    AbstractPlayerConnector playing =
        gh.getGame().getActivePlayer().equals(steve.getPlayer())
            ? steve
            : gh.getGame().getActivePlayer().equals(mike.getPlayer()) ? mike : alice;
